...
const ChatWrapper = ({ sessionId, initialMessages }: { sessionId: string, initialMessages: Message[] }) => {
    const { handleInputChange, handleSubmit, messages, input, setInput } = useChat({
        api: "/api/chat-stream",
        body: { sessionId },
        initialMessages
    })
    return (
        <div className='relative min-h bg-zinc-900 flex divide-y divide-zinc-700 flex-col justify-between gap-2'>
